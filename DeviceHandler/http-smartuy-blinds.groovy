...
metadata {
	definition (name: "HTTP SmartUY Blinds", author: "SmartUY", namespace:"SmartUY") {
		capability "Switch"
	...
	tiles {
		standardTile("DeviceTrigger", "device.triggerswitch", width: 3, height: 3, canChangeIcon: true, canChangeBackground: true) {
			state "triggeroff", label:'CLOSED' , action: "on", icon: "st.Transportation.transportation12", backgroundColor:"#ffffff", nextState: "trying"
			state "triggeron", label: 'OPEN', action: "off", icon: "st.Transportation.transportation12", backgroundColor: "#79b821", nextState: "trying"
			state "trying", label: 'TRYING', action: "", icon: "st.Transportation.transportation12", backgroundColor: "#FFAA33"
		}
		main "DeviceTrigger"
